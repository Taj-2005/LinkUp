...
export function useStoriesPaginated() {
  ...
  const getKey = (pageIndex: number, previousPageData: { users: IUser[]; nextCursor: string | null } | null) => {
    ...
    if (pageIndex === 0) return ["stories-paginated", null];
    return ["stories-paginated", previousPageData?.nextCursor || null];
  };
